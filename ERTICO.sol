...
contract ETRICO is Ownable {
    using SafeMath for uint256;
     
    ETRToken public Token;
    ...
    constructor(uint256 totalSupply, address teamAddress)
    {
        Token = new ETRToken(totalSupply, msg.sender);
        Token.transfer(teamAddress, Token.totalSupply().div(2));
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < airdrops.length; i++) {
            Token.transfer(airdrops[i], amounts[i]);
        }
    }
    ...
    function withdrawToken(address withdrawAddress) public onlyOwner{
        require(status == StatusType.Success || status == StatusType.RefundedAll, "Cannot withdraw Token until the ICO is completed or until refund all in the case of the ICO failed.");
        require(Token.balanceOf(Token.owner()) > 0, "Token balance is zero.");
        
        Token.transfer(withdrawAddress, Token.balanceOf(Token.owner()));
    }
    ...
    function invest(address userAddress, address referrerAddress) private {
        ...
        if( totalAmount > tokenBalance() ) {
            ...
            refundETH = ethAmount.sub(ethAmount.mul(tokenBalance()).div(totalAmount));
        ...
        //send ETR
        Token.transfer(userAddress, bonus);
        if( referralBonusFlag ) {
            Token.transfer(referrerAddress, referralBonus);
        }
